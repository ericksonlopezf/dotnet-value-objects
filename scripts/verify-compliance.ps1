...
foreach ($proj in $projFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "<NoWarn>.*(CS0618|CS0619|CS1591|CS0159|CS159\b|IDE1006).*</NoWarn>") {
            Write-Host "  ❌ Prohibited warning suppression ($Matches[1]) in $($proj.FullName):$($i + 1)" -ForegroundColor Red
            $violations++
            $illegalNoWarn++
        }
    }
}
if ($illegalNoWarn -eq 0) { Write-Host "  ✅ Zero prohibited NoWarn suppressions found (CS1591, CS0618, CS0619, CS0159, IDE1006)." -ForegroundColor Green }

# 8. Zero Pragma Warning Suppressions Across Codebase
Write-Host "`n[Gate: Pragma Suppression] Checking for '#pragma warning disable' directives across codebase..." -ForegroundColor Yellow
$pragmaViolations = 0
$allCsFiles = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.cs" | Where-Object { $_.FullName -notmatch '[\\/](obj|bin|StrykerOutput|\.git)[\\/]' }
foreach ($cs in $allCsFiles) {
    $lines = Get-Content $cs.FullName
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match '^\s*#pragma\s+warning\s+disable\b') {
            Write-Host "  ❌ Prohibited '#pragma warning disable' found in $($cs.FullName):$($i + 1)" -ForegroundColor Red
            $violations++
            $pragmaViolations++
        }
    }
}
if ($pragmaViolations -eq 0) { Write-Host "  ✅ Zero '#pragma warning disable' directives found across entire repository." -ForegroundColor Green }

# 9. ImplicitUsings Enforcement in Directory.Build.props
Write-Host "`n[Gate: Implicit Usings] Validating '<ImplicitUsings>disable</ImplicitUsings>' in Directory.Build.props..." -ForegroundColor Yellow
$bldPropsPath = Join-Path $RootDirectory "Directory.Build.props"
if (Test-Path $bldPropsPath) {
    $bldContent = Get-Content $bldPropsPath -Raw
    if ($bldContent -notmatch '<ImplicitUsings>\s*disable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Directory.Build.props must globally declare '<ImplicitUsings>disable</ImplicitUsings>'." -ForegroundColor Red
        $violations++
    } else {
        Write-Host "  ✅ ImplicitUsings disabled globally in Directory.Build.props (explicit namespace import policy)." -ForegroundColor Green
    }
}

# 10. Issue Template Kebab-Case & Assignee Governance
Write-Host "`n[Gate: Issue Templates] Validating kebab-case and maintainer assignees in .github/ISSUE_TEMPLATE..." -ForegroundColor Yellow
$tmplFiles = Get-ChildItem -Path (Join-Path $RootDirectory ".github/ISSUE_TEMPLATE") -File -ErrorAction SilentlyContinue
$tmplErrors = 0
if ($tmplFiles) {
    foreach ($tmpl in $tmplFiles) {
        if ($tmpl.Name -match '_') {
            Write-Host "  ❌ Non-kebab-case issue template: $($tmpl.FullName)" -ForegroundColor Red
            $violations++
            $tmplErrors++
        }
        $tmplContent = Get-Content $tmpl.FullName -Raw
        if ($tmplContent -match "assignees:\s*['""]ericksonlopez['""]") {
            Write-Host "  ❌ Outdated assignee 'ericksonlopez' in $($tmpl.FullName). Required: 'ericksonlopezf'." -ForegroundColor Red
            $violations++
            $tmplErrors++
        }
    }
}
if ($tmplErrors -eq 0) { Write-Host "  ✅ All issue templates satisfy kebab-case naming and @ericksonlopezf assignees." -ForegroundColor Green }

# 11. Security Policy Version Harmony Gate
Write-Host "`n[Gate: Version Harmony] Validating active 2.0.x version support in SECURITY.md..." -ForegroundColor Yellow
$secPath = Join-Path $RootDirectory "SECURITY.md"
if (Test-Path $secPath) {
    $secContent = Get-Content $secPath -Raw
    if ($secContent -notmatch '\*\*\s*2\.0\.x\s*\*\*\s*\|\s*:white_check_mark:') {
        Write-Host "  ❌ SECURITY.md must designate 2.0.x as the active supported release." -ForegroundColor Red
        $violations++
    } else {
        Write-Host "  ✅ SECURITY.md correctly reflects current active version 2.0.x." -ForegroundColor Green
    }
}
