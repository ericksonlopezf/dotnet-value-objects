...
public sealed class ValueObjectDefaultInitializationAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void CheckAndReport(SyntaxNodeAnalysisContext context, ITypeSymbol? typeSymbol, Location location)
    {
        ...
        bool isValueObject = typeSymbol.AllInterfaces.Any(i => i.Name is "IValueObject" or "IValueObject1");
        ...
        if (!isValueObject)
        {
            isValueObject = typeSymbol.GetAttributes().Any(a => a.AttributeClass?.Name is "ValueObjectAttribute" or "ValueObject");
        }
