...
public sealed class ValueObjectJsonConverterFactoryTests
{
    ...
    public void CreateConverter_CachesAllResolvedConverters()
    {
        ...
        // Money
        var m1 = factory.CreateConverter(typeof(Money), _options);
        var m2 = factory.CreateConverter(typeof(Money), _options);
        m1.Should().NotBeNull();
        m1.Should().BeSameAs(m2);

        // CurrencyCode
        var c1 = factory.CreateConverter(typeof(CurrencyCode), _options);
        var c2 = factory.CreateConverter(typeof(CurrencyCode), _options);
        c1.Should().NotBeNull();
        c1.Should().BeSameAs(c2);

        // Range<int>
        var rInt1 = factory.CreateConverter(typeof(Range<int>), _options);
        var rInt2 = factory.CreateConverter(typeof(Range<int>), _options);
        rInt1.Should().NotBeNull();
        rInt1.Should().BeSameAs(rInt2);

        // Range<decimal>
        var rDec1 = factory.CreateConverter(typeof(Range<decimal>), _options);
        var rDec2 = factory.CreateConverter(typeof(Range<decimal>), _options);
        rDec1.Should().NotBeNull();
        rDec1.Should().BeSameAs(rDec2);

        // Range<double>
        var rDbl1 = factory.CreateConverter(typeof(Range<double>), _options);
        var rDbl2 = factory.CreateConverter(typeof(Range<double>), _options);
        rDbl1.Should().NotBeNull();
        rDbl1.Should().BeSameAs(rDbl2);

        // Range<DateTime>
        var rDt1 = factory.CreateConverter(typeof(Range<DateTime>), _options);
        var rDt2 = factory.CreateConverter(typeof(Range<DateTime>), _options);
        rDt1.Should().NotBeNull();
        rDt1.Should().BeSameAs(rDt2);

        // Range<DateOnly>
        var rDo1 = factory.CreateConverter(typeof(Range<DateOnly>), _options);
        var rDo2 = factory.CreateConverter(typeof(Range<DateOnly>), _options);
        rDo1.Should().NotBeNull();
        rDo1.Should().BeSameAs(rDo2);

        // Range<long> (generic fallback branch)
        var rLng1 = factory.CreateConverter(typeof(Range<long>), _options);
        var rLng2 = factory.CreateConverter(typeof(Range<long>), _options);
        rLng1.Should().NotBeNull();
        rLng1.Should().BeSameAs(rLng2);

        // IParsable: Email
        var p1 = factory.CreateConverter(typeof(Email), _options);
        var p2 = factory.CreateConverter(typeof(Email), _options);
        p1.Should().NotBeNull();
        p1.Should().BeSameAs(p2);

        // SingleValueObject: Country
        var svo1 = factory.CreateConverter(typeof(Country), _options);
        var svo2 = factory.CreateConverter(typeof(Country), _options);
        svo1.Should().NotBeNull();
        svo1.Should().BeSameAs(svo2);
    }
